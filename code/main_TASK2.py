...
pos, pos_val, pos_test = get_POS_encoding(sentences_train, sentences_val,
                                          sentences_test, True)

model = BiRNN_LSTM_POS("word2vec",
                       word2vec_embedding_layers,
                       use_len_and_position=True)
model.train(X_train,
            y_train,
            X_val,
            y_val,
            pos_train=pos,
            pos_val=pos_val,
            abstractPosFeat_train=abstractPosFeat_train,
            abstractPosFeat_val=abstractPosFeat_val,
            load_model=True)
...
model = BiRNN_LSTM_POS("fastText",
                       fasttext_embedding_layers,
                       use_len_and_position=True)
model.train(X_train,
            y_train,
            X_val,
...
#### BiLSTM with POS and position
model = BiRNN_LSTM_POS("kerasEmbed",
...
#### ResNet1D with position
model = ResNet1D_model("kerasEmbed",
                       keras_embedding_layers,
                       use_len_and_position=True)
model.train(X_train,
            y_train,
