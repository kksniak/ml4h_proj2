...
class Embeddings:
    ...
    def __init__(self, embedding_type: Literal["TF-IDF", "word2vec", "fastText", "kerasEmbed"], sentences_train: list, sentences_val: list, sentences_test: list) -> None:
        self.embedding_type = embedding_type
        self.sentences_train, self.sentences_val, self.sentences_test = sentences_train, sentences_val, sentences_test

    def train(self, load_model: boolean = False) -> Tuple[np.array, np.array, np.array]:
    ...
    def gensim_model(self) -> None:
        self.X_train = [[word for word in str(text).split()] for text in self.sentences_train]
        self.X_val = [[word for word in str(text).split()] for text in self.sentences_val]
        self.X_test = [[word for word in str(text).split()] for text in self.sentences_test]
    ...
    def train_word2vec(self) -> None:
        ...
        if self.load_model:
            ...
            self.model = Word2Vec.load('code/embeddings_checkpoints/word2vec.model')
        else:
            print('Training word2vec Embedding...')
            self.model = Word2Vec(min_count=1, vector_size=self.embed_dim, seed=SEED)
            self.model.build_vocab(self.X_train)
            self.model.train(self.X_train, total_examples=len(self.X_train), epochs=20)
    ...
    def train_fastText(self) -> None:
        ...
        if self.load_model:
            ...
            self.model = FastText.load('code/embeddings_checkpoints/fasttext.model')
        else:
    ...
    def tokenise_and_pad(self) -> None:
        ...
        # pad training sets
        self.X_train = pad_sequences(self.X_train, maxlen=self.max_sentence_len, padding='post')
        self.X_val = pad_sequences(self.X_val, maxlen=self.max_sentence_len, padding='post')
        self.X_test = pad_sequences(self.X_test, maxlen=self.max_sentence_len, padding='post')


    def get_embedding_layer(self, mean_embedding: boolean = True) -> List[keras.layers.Layer]:
        if self.embedding_type == "TF-IDF":
        ...
        if self.embedding_type == "word2vec" or self.embedding_type == "fastText":
            ...
            embedding_layers = [Embedding(input_dim = vocab_size, output_dim=self.embed_dim, input_length=self.max_sentence_len, embeddings_initializer=Constant(embedding_matrix), trainable = False)]
            if mean_embedding:
                embedding_layers.append(Lambda(lambda x: backend.mean(x, axis=1)))

        elif self.embedding_type == "kerasEmbed":
            embedding_layers = [Embedding(input_dim = vocab_size, output_dim=self.embed_dim, input_length=self.max_sentence_len)]
            if mean_embedding:
                embedding_layers.append(Lambda(lambda x: backend.mean(x, axis=1)))
