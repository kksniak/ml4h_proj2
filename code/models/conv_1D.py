...
class Conv1D_model:
    ...
    def init_model(self, input_shape: Tuple):
        ...
        x = Conv1D(16, 4, activation="sigmoid")(x)
        x = MaxPooling1D(2)(x)
        x = Flatten()(x)
    ...
    def train(self, X_train: np.array, y_train: np.array, X_val: np.array, y_val: np.array, load_model: boolean):
        ...
        if load_model:
            print("Loading model...")
            self.clf = keras.models.load_model("code/models/models_checkpoints/conv_1D_" + self.embedding_type)
            return        
        ...
        print("Saving model...")
        self.clf.save("code/models/models_checkpoints/conv_1D_" + self.embedding_type)        
